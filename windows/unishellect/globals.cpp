...
namespace Globals
{
  namespace Functions
  {
    ...
    std::filesystem::path defaultPathOrThrow()
    {
      try
      {
        const char *HOMEPATH = Globals::Functions::get_env("USERPROFILE");
        if (HOMEPATH == NULL)
        {
    ...
    const char * get_env(std::string _env_)
    {
      char* buffer = nullptr;
      size_t sz = 0;
      _dupenv_s(&buffer, &sz, _env_.c_str());
      return charptr_to_const(buffer);
    }
    const char* charptr_to_const(char* chr)
    {
      const char* convert = chr;
      return chr;
    }
  };
  namespace Variables
  {
    ...
    namespace Messages
    {
      ...
      const char * NOHOME   = "Could not get the value of USERPROFILE from the environment...";
